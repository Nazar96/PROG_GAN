...
class PROG_GAN():
    ...
    def __create_g_output(self, layer_index):
        ...
        model = Sequential()
        model.add(Conv2D(self.n_chanels, kernel_size=3, padding='same', input_shape=input_shape))
    ...
    def __create_g_layer(self, layer_index):
        ...
        model = Sequential()
        model.add(Conv2D(self.filter_shape_list[layer_index], kernel_size=3, padding='same', input_shape=input_shape))
        model.add(BatchNormalization(momentum=0.8))
        model.add(ReLU())
        try:
            model.add(Conv2D(self.filter_shape_list[layer_index + 1], padding='same', kernel_size=3))
        except IndexError:
            model.add(Conv2D(self.filter_shape_list[layer_index], padding='same', kernel_size=3))
        model.add(BatchNormalization(momentum=0.8))
        model.add(ReLU())
    ...
    def __build_generator(self, n_levels):
        ...
        z = Input(shape=(self.latent_shape,), name='GEN_INPUT')
        x = self._G_input(z)
    ...
    def __build_discriminator(self, n_levels):
        ...
        self._discriminator.compile(self.optimizer, 'binary_crossentropy')
    ...
    def __build_combined(self):
        ...
        self._comb.compile(self.optimizer, 'binary_crossentropy')
    ...
    def train_discriminator(self, real_X, alpha=0):
        ...
        loss_real = self._discriminator.train_on_batch([real_X, alpha], 0.9*y_real)
